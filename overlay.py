...
from PyQt5.QtGui import QColor, QPainter, QPen, QCursor, QMouseEvent, QPixmap, QPainterPath, QTextCursor
                ...
                if active_provider == "Gemini":
                    ...
                    if self.task_type == "vision" and self.image_path and os.path.exists(self.image_path):
                        ...
                        try:
                            ...
                            chosen_nv_model = self.provider_models.get("nvidia", "nvidia/nemotron-3-ultra-550b-a55b")
                        ...
                        try:
                            chosen_nv_model = self.provider_models.get("nvidia", "nvidia/nemotron-3-ultra-550b-a55b")
                            extra_body = {}
                            if "nemotron" in chosen_nv_model.lower():
                                extra_body = {
                            ...
                            self.error_signal.emit(f"NVIDIA Text Error: {e}")
                            return
...
class VisionInterviewWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Resize image down to max width 1280px to speed up Llama Vision processing while retaining text legibility
            img.thumbnail((1280, 1280), Image.Resampling.LANCZOS)
            
            output_buffer = io.BytesIO()
            img.convert('RGB').save(output_buffer, format="JPEG", quality=60, optimize=True)
            ...
            prompt = (
                ...
                "- DETECT USER CODE INTENTION: Scan the active code editor on the screen. Auto-identify the programming language/framework in use. If there is a coding task or comment, solve the coding task directly. Present the implementation using different approaches (e.g., Approach 1: Iterative, Approach 2: Recursive/Optimized) and format each approach in separate, cleanly defined code blocks.\n"
                ...
                "- ABSOLUTE FORBIDDEN: NEVER invent hypothetical interviewer questions like 'Can you explain the code you've written' or 'What is the purpose of this script'. NEVER generate questions asking to explain, trace, or review the user's current code unless the interviewer explicitly asked it in the Transcribed Interviewer Audio. If only screen code is present, the QUESTION field must ONLY represent the raw coding problem or comment title (e.g., 'QUESTION: Implement Insertion Sort').\n"
                ...
                "- ONLY INTERVIEW QUESTIONS: You must ONLY respond if there is an active, explicit technical coding challenge statement on the screen, a new behavioral question verbally asked, or an active coding intention visible on the screen. Do NOT explain what is on the screen, do not summarize code files, and do not describe the IDE structure or terminal window.\n"
                "- DISTINGUISH BETWEEN INTERVIEWER VS USER: Do not misunderstand the user's own active code typing (such as solutions, logic comments, or functions they are writing) or the user's answers as questions asked by the interviewer. ONLY detect questions or prompts that originate from the INTERVIEWER (either verbally in the audio or as an official problem statement/coding test description on the screen).\n\n"
                "RULES:\n"
                ...
                "2. If the interviewer presents a new technical/coding question on the screen, synthesize the visual and spoken context.\n"
                ...
                "QUESTION:\n[The detected question/prompt (either from audio or screen)]\n\n"
                ...
                "CRITICAL: If the Transcribed Interviewer Audio has no question from the interviewer AND the screen image contains user code, IDE code boilerplate, empty files, or no active technical test prompt, you MUST reply EXACTLY with the single word: NO_QUESTION. Do not hallucinate questions or explain visual layout elements."
            )
            if self.voice_text:
            ...
            payload = {
                ...
                "max_tokens": 512,  # Reduced max_tokens to speed up Nvidia endpoint completion and reduce stream processing time
                "temperature": 0.2,
                "top_p": 1.0,
                "stream": True
            }
                        ...
                        if decoded_line.startswith("data:"):
                            ...
                            try:
                                ...
                                if chunk_text:
                                    ...
                                    # Do not emit stream chunks to chat to prevent scroll bouncing; wait for final parsed check
                            except Exception:
                                pass
...
class TransparentOverlay(QFrame):
    ...
    def init_settings_frame(self, parent_layout):
        ...
        nvidia_models = ["nvidia/nemotron-3-ultra-550b-a55b", "meta/llama-3.2-11b-vision-instruct", "nvidia/llama-3.1-nemotron-70b-instruct", "meta/llama3-70b-instruct"]
        saved_nv = self.provider_models.get("nvidia", "nvidia/nemotron-3-ultra-550b-a55b")
        ...
        self.usage_stats_lbl.setStyleSheet("color: #a78bfa; font-size: 11px; padding: 4px; background: rgba(139, 92, 246, 0.1); border-radius: 4px;")
    ...
    def calculate_usage_statistics(self):
        ...
        try:
            ...
            # Count requests sent TODAY (since midnight)
            today_counts = {"Gemini": 0, "Groq": 0, "OpenRouter": 0, "NVIDIA": 0}
            ...
            for s in self.sessions:
                for m in s.get('messages', []):
                    if m.get('role') == 'ai':
                        ...
                        # Fallback: Count all recent sessions or assume today
                        if "Gemini" in p: today_counts["Gemini"] += 1
                        elif "Groq" in p: today_counts["Groq"] += 1
                        elif "OpenRouter" in p: today_counts["OpenRouter"] += 1
                        elif "NVIDIA" in p: today_counts["NVIDIA"] += 1

            # Standard free API limits configuration and reset specifications
            # Gemini: 1,500 requests per day (RPD) - resets daily at 00:00 UTC (GMT)
            # Groq: 14,400 requests per day (RPD) - resets daily at 00:00 UTC (GMT)
            # OpenRouter: 200 requests per day (RPD for free endpoints to avoid rate limit bans) - resets daily at 00:00 UTC (GMT)
            # NVIDIA: 1,000 requests limit (Nemotron credits) - resets daily at 00:00 UTC (GMT)
            ...
            for p in ["Gemini", "Groq", "OpenRouter", "NVIDIA"]:
                ...
                stats.append(f"{p}: {remaining}/{limit} Left (Resets: {utc_reset_str})")
                
            self.usage_stats_lbl.setText(
                f"<b>📊 Daily Quota Status:</b> &nbsp;&nbsp; " + 
                " &nbsp;&nbsp; | &nbsp;&nbsp; ".join(stats)
            )
        except Exception as e:
    ...
    def reset_to_default_models(self):
        ...
        self.model_nv.setCurrentText("nvidia/nemotron-3-ultra-550b-a55b")
        self.save_settings()
    ...
    def highlight_code(self, code_text, lang_name, is_dark):
        try:
            ...
            style_name = "vs" if is_dark else "friendly"
    ...
    def toggle_interview_mode(self):
        ...
        if self.interview_mode:
            ...
            # Pre-calculate screen hash on startup to prevent initial static screen trigger
            try:
                scan_path = self.capture_stealth_image()
                if scan_path and os.path.exists(scan_path):
                    import hashlib
                    with open(scan_path, "rb") as f:
                        self.last_img_hash = hashlib.md5(f.read()).hexdigest()
            except Exception:
                pass
    ...
    def on_vision_chunk_received(self, chunk):
        # Streaming chunks bypassed to prevent printing draft thinking blocks to main chat
        pass
    ...
    def on_vision_interview_result(self, question, solution):
        ...
        # Stop execution immediately if NO_QUESTION was returned
        if "NO_QUESTION" in question or "NO_QUESTION" in solution:
            if hasattr(self, 'preview_popup') and self.preview_popup:
                self.preview_popup.status_label.setText("Waiting for next question...")
            return
            
        if hasattr(self, 'preview_popup') and self.preview_popup:
            self.preview_popup.status_label.setText(f"Identified: {question[:30]}...")
            
        # Format the combined response beautifully in a single AI bubble
        combined_response = (
            f"❓ **Question Detected:**\n{question}\n\n"
            f"🧠 **Thinking & Solution:**\n{solution}"
        )
        
        self.save_chat_message("ai", combined_response, "Stealth AI")
            
        # Perform a single high-fidelity markdown render in standard interaction UI bubbles
        self.render_current_session()
            
        # Save the solution to last_ai_code for manual ghost typing injection, but do NOT auto-trigger ghost mode
        self.last_ai_code = solution
        
    def on_interview_voice_error(self, err):
        if not getattr(self, 'interview_mode', False):
            return
        # Silence/Timeout can trigger this. Restart listening loop instantly.
        QTimer.singleShot(100, self.start_interview_listening)
    ...
    def capture_stealth_image(self):
        try:
            ...
            # Capture the entire desktop screen context rather than the transparent overlay window geometry
            screen = QApplication.primaryScreen().geometry()
            monitor = {"top": screen.y(), "left": screen.x(), "width": screen.width(), "height": screen.height()}
            
            with mss.mss() as sct:
                sct_img = sct.grab(monitor)
                scan_path = os.path.join(get_app_dir(), "interview_scan.png")
    ...
    def poll_mouse_position(self):
        try:
            ...
            gp = QCursor.pos()
            rx, ry, rw, rh = getattr(self, 'cached_geometry', (0, 0, 0, 0))
            is_inside = (rx <= gp.x() <= rx + rw and ry <= gp.y() <= ry + rh)
            
            # --- Inject click detection (works in ALL focus modes) ---
            if getattr(self, 'waiting_for_inject_click', False):
                # Debounce: skip the first ~300ms after inject was armed to avoid detecting the inject link click itself
                import time
                ...
                if time.time() - arm_time > 0.3:
                    if ctypes.windll.user32.GetAsyncKeyState(0x01) & 0x8000:
                        if not is_inside:
                            ...
                            if text_to_inject:
                                self.waiting_for_inject_click = False
                                # Capture the target window under the cursor right now
                                target_hwnd = ctypes.windll.user32.WindowFromPoint(POINT(gp.x(), gp.y()))
                                # Delay to let caret activate in target, then type via hardware keys
            ...
            if is_inside:
                ...
                if child:
                    ...
                    parent_widget = child
                    while parent_widget:
                        if (isinstance(parent_widget, QPushButton) or 
                            isinstance(parent_widget, QLineEdit) or 
                            isinstance(parent_widget, QTextBrowser) or
                            parent_widget == getattr(self, 'slider', None) or 
                            parent_widget == getattr(self, 'opacity_label', None)):
                            should_be_interactive = True
                            break
                        parent_widget = parent_widget.parent()
                        
                    if not should_be_interactive:
                        lx, ly = lp.x(), lp.y()
                        margin = 10
                        if (lx <= margin or lx >= rw - margin or
                            ly <= margin or ly >= rh - margin):
                            should_be_interactive = True
                            
            is_interactive = getattr(self, 'temp_interactive', False)
            if should_be_interactive != is_interactive:
                self.temp_interactive = should_be_interactive
                self.set_window_interactive(should_be_interactive)
        except Exception:
            pass
