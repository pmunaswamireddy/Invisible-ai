...
class CustomWebEnginePage(QWebEnginePage):
    def __init__(self, parent_tab):
        # Create an off-the-record (incognito) profile to prevent session/tab restore
        profile = QWebEngineProfile(parent_tab.browser)
        profile.setPersistentCookiesPolicy(QWebEngineProfile.NoPersistentCookies)
        # Set a standard Chrome user-agent so sites serve full CSS/JS
    ...
    def createWindow(self, type_):
        new_tab = self.parent_tab.parent_overlay.create_empty_browser_tab()
    ...
    def paintEvent(self, event):
        ...
        for i in range(self.count()):
            if self.tabText(i) == "+":
                ...
                # Draw subtle hover circle
                if is_hovered:
                    painter.setBrush(QColor(255, 255, 255, 30))
                    painter.setPen(Qt.NoPen)
                    painter.drawRoundedRect(rect.adjusted(2, 2, -2, -2), 4, 4)
                
                # Draw plus icon lines
                ...
                option = QStyleOptionTab()  # Create a fresh option for each tab to prevent C++ memory crashes
                self.initStyleOption(option, i)
                painter.drawControl(QStyle.CE_TabBarTab, option)

class WebBrowserTab(QWidget):
    def __init__(self, url, parent_overlay):
        super().__init__()
        ...
        # Enable proper web rendering settings
    ...
    def _handle_browser_action(self, url_str):
        ...
        if url_str == "browser://clear-history":
            ...
            m = _re.match(r"browser://delete-history-(\d+)", url_str)
            if m:
                idx = int(m.group(1))
            ...
            m = _re.match(r"browser://delete-bookmark-(\d+)", url_str)
            if m:
                idx = int(m.group(1))
            ...
            m = _re.match(r"browser://close-tab-(\d+)", url_str)
            if m:
                idx = int(m.group(1))
                if idx > 0:
            ...
            m = _re.match(r"browser://switch-tab-(\d+)", url_str)
            if m:
                idx = int(m.group(1))
                tab_widget = self.parent_overlay.tab_widget
    ...
    def show_search_menu(self):
        ...
        def _se_icon(name):
            ...
            p = _os.path.join(_icons_dir, name)
            return QIcon(p) if _os.path.exists(p) else QIcon()
                        ...
                        if count > 0:
                            ...
                            if gemini_key:
                                try:
                                    ...
                                    # Relax safety block thresholds to avoid NoneType splitlines / safety block errors
                                    safety_settings = {}
                                    try:
...
class TransparentOverlay(QFrame):
    ...
    def __init__(self):
        ...
        self.browser_btn.setToolTip("Open a new web browser tab in the chat area")
    ...
    def create_empty_browser_tab(self):
        new_tab = WebBrowserTab(None, self)
    ...
    def open_in_mini_browser(self, url_str):
        new_tab = WebBrowserTab(url_str, self)
    ...
    def keyPressEvent(self, event):
        ...
        if getattr(self, 'leader_active', False):
            if vk == Qt.Key_Escape:
                ...
                event.accept()
                return
            elif vk == Qt.Key_B or vk == Qt.Key_W:
                self.toggle_browser_visibility()
                event.accept()
                return
            elif vk == Qt.Key_V:
