...
# 2. Patch nativeEvent
native_old_regex = r'    def nativeEvent\(self, eventType, message\):\s*if eventType == "windows_generic_MSG":\s*msg = ctypes\.wintypes\.MSG\.from_address\(int\(message\)\)\s*if msg\.message == 0x0312: # WM_HOTKEY'
...
text, count = re.subn(native_old_regex, native_new, text)
print(f"Patched nativeEvent {count} times.")
