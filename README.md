...
### 2. 👻 Ghost / Background Click-Through Mode *(New in v6)*
...
* **Smart Coordinate Interactivity (WM_NCHITTEST):** Replaces basic OS transparency with intelligent hit-testing. The background remains 100% transparent and click-through to your exam software, while specific overlay elements (Chat Scrollbar, Opacity Slider, and Chat Anchor Links like Copy/Inject) intercept clicks locally using `WM_MOUSEACTIVATE` blocks.
* **Undetectable:** The ghost window is physically invisible to the OS event system when clicking the background. When clicking interactive overlay elements (like the scrollbar), the overlay intercepts the click without *ever* stealing keyboard focus or triggering a focus-loss event in the exam software.
* **Multi-Digit Command Buffer:** In Command Mode, typing digits builds a buffer (e.g. `1` + `0` = `10`) for 400ms, allowing seamless injection of multi-digit code blocks.
...
### 8. 🌐 Stealth Mini Browser *(New in v5)*
* **Integrated Chromium Engine:** A lightweight, built-in mini browser optimized for stealth usage alongside coding IDEs and web applications.
* **Pure Incognito By Default:** Fully off-the-record profile. No cookies, no local storage, and no history tracking persists to disk across application restarts.
* **Resource Optimized:** Browser launches with disabled GPU rendering, no software rasterizer, and single-threaded parsing (`--disable-gpu`, `--num-raster-threads=1`) to eliminate system stutter and ensure absolutely zero footprint.
* **Modern Tab Interface:** Custom-painted Chrome-like `+` tab button, slim hover layouts, and close tabs individually or all at once.
* **High-Quality Search:** Address bar integration featuring authentic engine icons for **Google, DuckDuckGo, Mojeek, Brave, and Startpage**.
* **Gestures & Native Support:** Horizontal mouse scroll gestures for Back/Forward navigation. Direct Download Manager that pops open an OS folder selector to save files securely to disk, plus automatic User-Agent desktop overrides.
...
## 🛠️ Build and Sign Process (Windows 10 / 11)
...
### How to Build & Auto-Sign:
```bat
.\build_exe.bat
```
1. Activates `.venv`, compiles via PyInstaller with `--icon=overlay_icon.ico --noconsole --onefile`.
2. Packages the UI and dependencies into a single hidden file.

```powershell
.\self_sign.ps1
```
1. Automatically detects the compiled `SystemAudioEngine.exe`.
2. Generates a Trusted Root Certificate locally and signs the binary.
3. Registers the certificate into Windows Trusted Publishers, completely bypassing SmartScreen warnings.
